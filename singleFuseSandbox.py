...
from fuseManager import mount_fuse_filesystem, unmount_fuse_filesystem
...
def launch_application(app_command, policy_file, default_policy_file, username, mount_dir):
    ...
    try:
        ...
        policy_ini_path = create_policy_ini(policy_file, mount_dir, debug_mode, "single")

        # Setup the FUSE filesystem using infuser.py
        mount_fuse_filesystem(mount_dir, policy_ini_path, policy_data, debug_mode)

        # Build the Bubblewrap command from the policy
        bwrap_command = build_bubblewrap_command(policy_data, app_command, mount_dir)
