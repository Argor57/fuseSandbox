...
def parse_policy(pf: str):
    ...
    values_section = ["read", "write", "execute", "written-no-execute",
                      "first-object-only", "first-object-only-recursive"]  # Values for names of the sections

    # Policies that do not allow the use of wildcards
    wildcard_prohibited_policies = ["first-object-only"]
    ...
    for sec in parser.sections():
        if sec not in values_section:
            ...
            continue
        # If a wildcard is in a policy that does not allow this, abort
        if sec in wildcard_prohibited_policies and [entry for entry in parser.items(sec) if "*" in entry[0]]:
            print(f"Wildcards are not allowed for policy section {sec}.")
            sys.exit(1)
